...
- 대상 저장소가 사용 중인 **엔진 버전을 커밋 가능한 형태로 고정**한다(작업 전에는 버전 표식이 전혀 없었다).
...
해석기는 선택한 경로와 함께 출처(`env`/`git`/`cwd`)를 반환한다. handoff는 이를 run metadata의 `repoRootSource`에 기록하고, task CLI는 세 후보 어디에도 `TASK.md`가 없으면 최종 탐색 경로와 `TASK_STATE_FILE` 지정 방법을 오류에 표시한다. 별도의 `--verbose` 옵션은 없다.
...
- 기존 `tools/task/verify.local.json`은 더 이상 읽지 않는다. 사용 중이었다면 사람이 `.duet/verify.json`으로 옮긴다([release-checklist.md §6](release-checklist.md#section-6)). 설치기는 사용자 설정을 자동 이동하거나 삭제하지 않는다.

### 3.5 테스트 경로 (B5)

`engine/task/test/helpers.js`가 설치 사본이 아니라 **`engine/`을 직접** 대상으로 삼도록 바꿨다(`path.resolve(__dirname, '../index.js')`). 따라서 엔진 테스트를 위해 자기설치할 필요가 없고 저장소 루트의 `npm test`가 task·handoff·installer 러너를 차례로 실행한다. `node --test engine/*/test/*.test.js` 같은 셸 glob은 Windows와 지원 Node 버전에서 동작이 갈리므로 사용하지 않는다.
...
> **구현 결과**: 단일 소스는 `package.json`의 `version`이 됐다(`engine/task/index.js`의 `--version`이 그것을 읽는다). `.claude-plugin/plugin.json`은 소스가 아니라 동기화 대상이며, `scripts/sync-version.js`가 설치 스펙들과 함께 맞춘다 — [release-checklist.md §8](release-checklist.md#section-8).
...
## 5. 당시 마이그레이션 계획 (완료된 역사 기록)

> 아래는 외부화 당시 실제로 사용한 전환 순서의 기록이다. 현재 설치기에는 `--engine-only`·`--force`가 없고 대상 저장소에 엔진 테스트 실행 스크립트도 구성하지 않으므로, **현재 업그레이드 절차로 실행하면 안 된다.** npm 패키지 안에는 테스트 파일이 포함될 수 있다. 현재 마이그레이션 방법은 [release-checklist.md §6](release-checklist.md#section-6)을 따른다.

당시 실사용 대상은 초기 설치 저장소 1곳뿐이었다. 파괴적 전환을 피하기 위해 다음 순서로 진행했다.
...
| 3 | [pipeline-design.md §2](pipeline-design.md#section-2) — *"최종 방어선은 커밋 전 사람의 `git diff TASK.md`"* | **DONE 승인 게이트의 최종 방어선이 소멸.** `approve-partial`의 TTY 검사는 설계상 신원을 보증하지 않으며, 그 공백을 git diff가 메운다 |
...
3. ~~기존 `tools/` 레이아웃 지원을 언제 끊을 것인가~~ → **설치·업그레이드 경로의 지원은 종료했다.** 런타임의 위치 독립성 때문에 체크아웃된 엔진 코드는 임의 경로에서도 동작하지만, `duet-init`은 더 이상 `tools/`에 배치하거나 그 레이아웃을 갱신하지 않는다. 남은 잔재는 **보고만** 하고 지우지 않는다(사용자 파일을 지우는 것은 설치기의 권한이 아니다).
...
- [x] 대상 소유 엔진 사본(`tools/`)이 **아예 생기지 않는다** — npm이 관리하고 이미 gitignore되는 `node_modules/duetcode`만 존재한다.
