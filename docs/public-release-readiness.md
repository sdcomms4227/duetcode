...
최초 검토는 18커밋 시점이었다. 그 뒤 7커밋(설치본 업그레이드 수정 → 엔진 외부화 → 문서 정합성 → 검증 증거)이 쌓였으므로, **public push는 되돌릴 수 없으니** 그 구간을 같은 패턴으로 다시 훑었다(당시 기준점 `e53438a`. 이후 §3.1의 픽스처 수정으로 이력을 재작성해 SHA가 전부 바뀌었으므로 이 값으로는 더 이상 조회되지 않는다).
