...
**재검토 조건은 둘 중 하나가 성립할 때다:**

1. **대상 저장소가 2곳 이상이 되거나 semver 범위가 필요해질 때.** `github:` 스펙은 `^0.2.2` 같은 범위를 쓸 수 없어 업그레이드가 매번 태그를 손으로 바꾸는 작업이 된다. 대상이 늘면 이게 실제 비용이 된다.
2. **git 없는 빌드 환경에서 설치해야 할 때.** `github:` 스펙은 설치 시점에 git이 필요해 슬림한 도커 이미지나 일부 CI 러너에서 깨진다.

> 보류의 비용은 하나뿐이고 작다 — `duetcode` 이름은 npm에 아직 미등록이지만(`v0.2.2` 시점 404 확인) 누가 선점하면 되돌릴 수 없다. 이 리스크만을 이유로 발행한다면 그건 기술적 필요가 아니라 **이름 선점**이라는 별개 판단이다. 반대로 발행은 사실상 비가역이다(72시간 내에만 unpublish, 같은 버전 재발행 영구 불가).

**발행하기로 뒤집을 때 같이 해야 하는 일**: `scripts/sync-version.js`는 지금 README·설치 스킬·스니펫에 `github:...#v<버전>` 스펙을 써 넣는다. 레지스트리 스펙으로 바꾸려면 그 치환 패턴도 함께 고쳐야 한다 — 안 고치면 다음 릴리스에서 패턴을 못 찾아 스크립트가 실패한다(§8). 패키지 자체는 발행 준비가 끝나 있다(`npm publish --dry-run`으로 `files`에 문서 포함까지 확인).
