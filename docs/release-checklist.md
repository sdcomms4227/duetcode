...
> **이 저장소의 "배포"는 태그 push까지다. npm 발행은 하지 않는다**(확정 근거는 [§7의 결정 기록](#npm-발행--하지-않는다-2026-07-29-확정)). 릴리스 작업 중에 npm publish를 절차의 다음 단계로 꺼내지 않는다 — 대상 저장소는 `github:sdcomms4227/duetcode#<tag>` 스펙으로 설치하므로 태그가 게시되면 그 시점에 배포가 끝난 것이다.

`https://github.com/sdcomms4227/duetcode` public. 최신 **게시 완료** 릴리스는 `v0.4.2`다. 커밋·태그·push는 매 릴리스마다 변경 검토가 끝난 뒤 사람이 수행하는 게이트로 남는다.
...
### v0.4.1 → v0.4.2 — 결정이 결정을 읽지 않는 자리에만 적혀 있었다

**patch로 올린 이유**: 코드 변경이 없다. 문서만 바뀌었고 공개 계약·엔진 동작은 그대로다. 그럼에도 릴리스로 올리는 이유는, 대상 저장소가 `github:` 태그로 설치하므로 태그가 없으면 이 문서가 어떤 설치에도 닿지 않기 때문이다.

**"npm 발행을 하지 않는다"는 확정 결정이 §7 안쪽에만 있었다.** v0.4.1 배포 작업 중에 실제로 그 결정을 읽지 못한 채 npm publish가 절차의 다음 단계로 제안됐다(인증이 없어 실행되지는 않았다). 결정 자체는 2026-07-29에 조건부 재검토 없이 확정돼 있었지만, 릴리스 작업자가 실제로 읽는 자리 — 절차의 맨 앞과 `CLAUDE.md` — 에는 없었다.

> **기록의 위치는 기록의 존재와 다르다.** 이 저장소는 재론을 막으려고 결정의 근거를 길게 적어 왔는데, 그 방식은 "누군가 그 절을 편다"를 전제한다. 릴리스 절차를 따라가는 사람(또는 에이전트)은 §1부터 읽지 §7을 펴지 않는다. 그래서 근거는 §7에 두되, **금지 그 자체는 절차의 첫 화면과 저장소 규칙 파일 양쪽에** 한 줄로 두고 근거로 링크했다. 형태는 v0.4.1의 수정들과 같다 — 고쳤다고 적은 것이 실제로 닿는 범위를 확인한 것이다.

`docs/release-checklist.md` §1 맨 앞과 `CLAUDE.md`의 릴리스 문단에 "배포는 태그 push까지, npm 발행 없음"을 넣었다. `CLAUDE.md` 쪽은 실행뿐 아니라 **제안도 하지 말 것**을 명시한다 — 이번에 새어나온 지점이 실행이 아니라 제안이었다.
