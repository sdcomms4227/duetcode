...
기능 요청 접수 → Claude 설계 → Codex 구현 → Claude 리뷰 → 사용자 승인·Git 반영까지 하나의 예시로 설명한다. 상태와 실시간 핸드오프는 저장소 루트의 `TASK.md`에 기록하며, 상태 변경은 `node tools/task/index.js`로 수행한다.
...
```bash
node tools/task/index.js start production-year-filter \
...
```bash
node tools/task/index.js set roles.implementer=gpt-5.6-sol
node tools/task/index.js set "roles.reviewer=Opus 4.8"
node tools/task/index.js set designCheckpoint=<설계-커밋-SHA>
...
```bash
node tools/task/index.js set status=IMPLEMENTING        # 구현 보완
node tools/task/index.js set status=READY --design-checkpoint <새-SHA>   # 설계 변경
```
...
node tools/task/index.js record-verification --status PARTIAL --failed-count 0
```

PARTIAL로 완료하려면 사용자가 실제 터미널에서 승인한다.

```bash
node tools/task/index.js approve-partial
...
```bash
node tools/task/index.js reset
```

## 9. 예외 흐름

```bash
node tools/task/index.js block "테스트 계정 발급 대기"
node tools/task/index.js unblock          # 차단 직전 활성 상태로 복귀
node tools/task/index.js cancel "우선순위 변경으로 중단"
node tools/task/index.js supersede other-task-id "요구사항 전면 변경"
```
...
- 옵션이 있는 명령은 PowerShell/npm 조합에서 옵션이 제거될 수 있으므로 `node tools/task/index.js ...` 직접 실행을 권장한다.
