...
Neither engine infers anything from its own path, so both behave identically under `node_modules/`, a checkout, or anywhere else. Three rules live in `engine/task/lib.js` because both engines need them and a second copy would let the two drift: `resolveRepoRoot` (below), `terminateProcessTree` (kill a spawned process *tree* — handoff uses it for codex, `task verify` for its smoke server; killing only the parent leaves the grandchild holding the port). **Tree kill is platform-split and POSIX needs the caller's cooperation**: Windows gets `taskkill /T`, while POSIX has no way to name "all descendants" and must kill the process *group* — which only works if the child was spawned `detached: true`, so the caller passes `{ group: true }` to say so. Never attempt `kill(-pid)` speculatively: if that pid happens to match another group's pgid you kill someone else's processes, and in CI that can be the runner itself. `task verify` spawns detached and passes the flag; the handoff dispatcher does not create a group yet, so on POSIX it still only kills codex itself (pre-existing, and the reason the flag is explicit rather than inferred), and `resolveSpawn` (make a command spawnable per platform — Windows `.cmd`/`.bat` cannot run without a shell, Node blocks it with `EINVAL`, while `shell: true` merely concatenates args and breaks paths with spaces, so it wraps in `cmd.exe /d /s /c` with our own quoting plus `windowsVerbatimArguments`). When you need one of these, import it — do not re-implement it next to the caller.
