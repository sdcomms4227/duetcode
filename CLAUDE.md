...
Neither engine infers anything from its own path, so both behave identically under `node_modules/`, a checkout, or anywhere else. Three rules live in `engine/task/lib.js` because both engines need them and a second copy would let the two drift: `resolveRepoRoot` (below), `terminateProcessTree` (kill a spawned process *tree* — handoff uses it for codex, `task verify` for its smoke server; Windows has no process groups, so killing the parent leaves the grandchild holding the port), and `resolveSpawn` (make a command spawnable per platform — Windows `.cmd`/`.bat` cannot run without a shell, Node blocks it with `EINVAL`, while `shell: true` merely concatenates args and breaks paths with spaces, so it wraps in `cmd.exe /d /s /c` with our own quoting plus `windowsVerbatimArguments`). When you need one of these, import it — do not re-implement it next to the caller.
