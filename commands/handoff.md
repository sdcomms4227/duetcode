...
allowed-tools: Bash(npm run handoff:*), Bash(node tools/handoff/dispatch.js:*), Bash(node tools/task/index.js:*), Bash(git status:*), Read
---
...
- dispatcher는 자동 재시도·DONE 전환·rollback을 하지 않는다. 실패 시 실측 상태와 로그 경로(`tools/handoff/state/runs/<runId>/`)를 사용자에게 보고하고 정지한다.
