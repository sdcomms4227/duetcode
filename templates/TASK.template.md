...
- 최상단 YAML front matter = 기계-판독 상태(단일 소스). 상태 전환·lint는 `node tools/task/index.js`로만 수행한다.
- 그 아래 프로즈 = 사람용 상세(요구사항·근거·검증 로그).
- 동시에 하나의 Active Task만 유지한다.

## Active Task

(IDLE — 활성 Task 없음. `node tools/task/index.js start <id> --objective <목표> --requester <요청자> --designer <설계자>`로 시작한다. start가 이 아래 본문을 새 Task 스켈레톤으로 교체한다.)
