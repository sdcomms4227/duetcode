...
function buildPrompt(options = {}) {
	...
	return [
		...
		'- ' + taskFile + ' 본문을 고칠 때는 편집기로 해당 절만 직접 수정한다. 문자열 치환 API로 본문을 갈아끼우지 않는다 —',
		'  JS String.replace의 치환 문자열에서 $& $` $\' $1 은 특수 토큰이라 새 본문에 그런 문자가 있으면 문서가 복제·훼손된다',
		'  (실제로 $` 가 문서 앞부분 전체로 치환돼 ' + taskFile + ' 가 통째로 복제된 사고가 있었다).',
		'- 본문을 고친 뒤에는 npm run task -- lint 로 문서가 온전한지 확인한 다음 다음 단계로 넘어간다.',
